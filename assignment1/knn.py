...
for k in k_choices:
    ...
    for f in range(num_folds):
        knn.train(np.concatenate(X_train_folds[0:f] + X_train_folds[f + 1:num_folds]),
                  np.concatenate(y_train_folds[0:f] + y_train_folds[f + 1:num_folds]))
        pre = knn.predict(X_train_folds[f], k=k)
        acc.append(np.mean(np.array([pre==y_train_folds[f]])))
